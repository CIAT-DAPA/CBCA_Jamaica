#### CBCA Jamaica - Make project files - (PRM)
...
## load function for write projects 

## tidy data - create lits of params

## Parallel function 

## Write Aquacrop Projects
...
##Change s1 and s2
bl_data_to_project <- wdata %>% 
  mutate(id_name = str_replace(Site, "_", ""),
         co2 = "MaunaLoa.CO2",
         id2 = "S2",
         sowing_dates = map(data, ~make_hist_dates(9, 10, .x))) %>% unnest(sowing_dates) %>% 
  mutate(soil = list(c("Clay", "ClayLoam", "Loam", "SandyLoam", "Sand")),
         max_crop_duration = 160, 
         cultivar = list(c("SweetPotatoGDD", "TomatoGDD", "GroundnutGDD")),
         aquacrop_files = "D:/03_DEVELOPER/CBCA_Jamaica/aquacrop_files/",
         plugin_path = "D:/03_DEVELOPER/CBCA_Jamaica/plugin/") %>%
  unnest(cultivar) %>% unnest(soil) %>% rename(clim_data = data) %>% 
  select(id_name, sowing_dates, cultivar, soil, clim_data, co2, id2, max_crop_duration, aquacrop_files, plugin_path)

# from ccdata
a %>% select(id_name, id2, soil, cultivar) %>% write_csv("id_escenarioscc.csv")
...
#tictoc::tic()
parLapply(cl, bl_data_to_project %>% split(.,1:nrow(.)), function(x){ 
...
dist_files("plugin/LIST/", "plugin2/LIST/", "Amityhall")
dist_files("plugin/LIST/", "plugin3/LIST/", "Parnassus")

gc()

## ----> Run each plugin folder

### 9. Ejecuta Aquacrop mediante el PlugIng
#tictoc::tic()
#walk(paste0(path_pi[1], "ACsaV60.exe"), ~system(.x))
##4278.01 sec elapsed
##2823.87 sec elapsed
#tictoc::toc()
